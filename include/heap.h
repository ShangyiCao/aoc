...
Heap *Heap_create(enum HeapType type, size_t sizeOfKey, size_t sizeOfValue,
                  int (*compare_value)(const void *, const void *),
...
void Heap_heapifyUp(Heap *heap, int index) {
    ...
    switch (heap->type) {
        case MAX:
            swap = heap->compare_value(heap->table[index]->value,
...
void Heap_heapifyDown(Heap *heap, int index) {
    ...
    switch (heap->type) {
        ...
        case MIN:
            ...
            if (min != index) {
                HeapItem *tmp = heap->table[index];
                heap->table[index] = heap->table[min];
                heap->table[min] = tmp;
                Heap_heapifyDown(heap, min);
            }
            break;
        default:
            perror("Unknow heap type!\n");
